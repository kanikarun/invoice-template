...
export class BoldStripePictureInvoiceDocument extends BaseInvoiceDocument {
  ...
  private readonly defaultStyle: Style = { color: '#222', fontSize: 8, font: 'Khmer' };
  ...
  private header(): Content {
    ...
    const content: Content = [
      svg,
      { margin: [0, 15, 0, 6], columns: [image, { alignment: 'right', stack: [title, invoiceNo, invoiceDate] }] },
      { alignment: 'left', stack: [merchant, { text: header }] }
    ];
  ...
  private khqr_table(): Content {
    ...
    return {
      ...
      table: { widths: ['auto'], body: [[{ image: 'KHQR', alignment: 'center', fit: [200, 70] }]] }
    };
  }
  ...
  private price_cell(
    ...
    return {
      ...
      columns: [
        ...
        { text: [this.currency_format(Math.abs(value), currency)], width: 'auto', marginTop }
      ]
    };
  }
  ...
  private price_row(opt: PriceRow): TableCell[] {
    ...
    const img_row: TableCell = is_img_row
      ? {
          colSpan: 3,
          // total + discount + delivery + tax + sub total + exchange rate + deposit + empty row
          rowSpan: 1 + r1 + r2 + r3 + r4 + r5 + r6 + 1,
          border,
          layout: {
            hLineWidth: () => 0,
            vLineWidth: () => 0,
