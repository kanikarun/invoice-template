...
export class CosmeticIntlInvoiceDocument extends BaseInvoiceDocument {
  ...
  private readonly styles: Record<string, Style> = {
    the_title: { fontSize:15, alignment: 'right', color: this.soft_color},
    inv_title: { fontSize: 15,marginTop:-6, bold: true, alignment: 'center', color: this.soft_color},
    tbl_title: { alignment: 'center', bold: true, color: '#e0e0e0',fontSize:7,marginLeft:4 },
    tbl_total: { bold: true, color: this.soft_color }
  };

  private readonly defaultStyle: Style = {
    color: this.soft_color,
    fontSize: 6.5,
    font: 'Khmer',
  };
  ...
  private header(): Content {
    ...
    if (this.merchant_invoice_tin) merchantStack.push({ text: `VATTIN : ${this.merchant_invoice_tin}`, marginTop: 3 });
    if (this.merchant_telephone) merchantStack.push({ text: `Phone No : ${this.merchant_telephone}` });
  ...
  private info_table(): Content {
    ...
    if (phone && phone !== '-') body.push([{ text: 'Phone No' ,noWrap: true }, ` : ${phone}`]);
  ...
  private menu_table(): Content {
    ...
    const header_row: TableCell[] = [
      { stack: [ this.getRoundedEdge({ x: -8, y:-5 }, {color: this.soft_color,w: 365, h: 20 }),],},
      ...
      { style: 'tbl_title', text: 'Amount' , marginLeft:11}
    ...
    const subTotal_text = this.no_sub_total ? null : this.price_row({ title: this.no_deposit_price ? 'Sub Total' : 'Balance Due', value: sub_total, border_top: true });
 ...
 private signature_table(): Content {
                ...
                table: {
                  ...
                  body: [
                    ...
                    [
                      {
                        canvas: [
                          { type: 'line', lineColor: this.soft_color, x1: 0, y1: 0, x2: 100, y2: 0, lineWidth: 0.5 }
                        ]
                      },
                      '',
                      {
                    ...
                    [
                      {
                        stack: [
                          { text: 'Customer', alignment: 'center', fontSize: 6 },
                      ...
                      {
                        stack: [
                          { text: 'Seller', alignment: 'center', fontSize: 6 },
  ...
  async getDefinition(): Promise<TDocumentDefinitions> {
    ...
    let patternSvg = readFileSync(
      path.join(process.cwd(), 'public', 'pinkie.svg'),
      'utf8'
    );

    patternSvg = patternSvg      //color SVG pattern
      .replace(/#d94e6c/gi, this.background_color)

    let edgeSvg = readFileSync(
      path.join(process.cwd(), 'public', 'pinkie_edge.svg'),
      'utf8'
    );
    edgeSvg = edgeSvg.replace(/#d94e6c/gi, this.background_color);

    let sunSvg = readFileSync(
      path.join(process.cwd(), 'public', 'sun.svg'),
      'utf8'
    );
    sunSvg = sunSvg.replace(/#d94e6c/gi, this.background_color);
    ...
    return {
      ...
      background: () => ([
        ...
        { svg: edgeSvg, fit: [180, 180], absolutePosition: { x: -50, y:490 } },
        { svg: sunSvg, fit: [80, 80], absolutePosition: { x: 340, y: 0} }
      ]),
