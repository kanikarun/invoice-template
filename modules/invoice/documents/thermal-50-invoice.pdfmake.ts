...
export class Thermal50InvoiceDocument extends BaseInvoiceDocument {
  ...
  private readonly styles: Record<string, Style> = {
    the_title: { alignment: 'center', fontSize: 6 },
    inv_title: { alignment: 'center', fontSize: 6, bold: true, decoration: 'underline' }
  };
  ...
  private header(): Content {
    const image: TableCell = { width: 20, height: 20, image: 'LOGO', rowSpan: 2 };
    ...
    if (this.merchant_telephone) {
      header.push({ text: `លេខទូរស័ព្ទ: ${this.merchant_telephone}`, alignment: 'center', fontSize: 5.5 });
    }
    ...
    return {
      ...
      table: {
        ...
        body: [
          [image, merchant, ''],
          ['', title, ''],
          [{ colSpan: 3, stack: header, alignment: 'center', fontSize: 5.5 }, '', '']
        ]
      }
    };
  }
  ...
  private info(): Content {
    ...
    const address = _address ? { text: `អាស័យដ្ឋាន៖ ${_address.replaceAll('\n', ' ')}`, fontSize: 5.5 } : null;
  ...
  private table(): Content {
    ...
    const exchange_rate_text = `សរុបរួមរៀល | In Riel`;
    ...
    return {
      ...
      table: {
        widths: ['*', 'auto'],
        body: [
          [this.table_header_cell('Item Name', 'ឈ្មោះទំនិញ'), this.table_header_cell('Amount', 'តម្លៃ', 'right')],
  ...
  private table_header_cell(en_title: string, kh_title: string, alignment?: 'right'): Content {
    return [
      { text: kh_title, alignment, bold: true },
      { text: en_title, alignment, bold: true }
    ];
  }
  ...
  private note(): Content {
    ...
    return { stack: htmlToPdfmakeText(`ចំណាំ | Note: ${this.data.invoice.note}`) };
  }
